...
temp <- sqlQuery(chan, "select top 10 * from [PRODW].[ORG_TREE_DRAD]")


#query for Oracle
library("RODBC", lib.loc="~/R/win-library/3.4")
chan = odbcConnect("SBP416.world",uid=username,pwd=pswd)
sqlQuery(chan, "select * from APPCA.D_STORE_VERS")

#using keyringr
library("RODBC", lib.loc="~/R/win-library/3.4")
psswd <- .rs.askForPassword("Database Password:")
chan = odbcConnect("SBP416.world",uid="JUMORRIS",pwd=psswd)
sqlQuery(chan, "
         SELECT DISTINCT sr.QSTN_ID
         ,ca.FSCL_YR_NUM
